...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        # Instantiate in order: word embeddings, position embeddings, transformer blocks, final layer norm, vocab projection
        ...
        for _ in range(num_blocks):
            self.layers.append(self.TransformerBlock(model_dim, num_heads))
        
        self.layers.append(nn.LayerNorm(model_dim))
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        tensor = self.word_embeddings(context) + self.position_embeddings(positions)
        
        # 2 & 3. Pass through transformer blocks + final LayerNorm (both in self.layers)
        tensor = self.layers(tensor)
        
        # 3. Project to vocab_size
        tensor = self.vocab_projection(tensor)
        
        # 4. Softmax and round
        res = torch.nn.functional.softmax(tensor, dim=-1)
